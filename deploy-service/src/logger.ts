...
export const logger = {
    ...
    buildLog: (id: string, type: 'STDOUT' | 'STDERR', data: string) => {
        const color = type === 'STDERR' ? '\x1b[31m' : '\x1b[37m';
        const lines = data.trimEnd().split('\n');
        for (const line of lines) {
            if (line.trim().length > 0) {
                console.log(`\x1b[36m[${getTimestamp()}]\x1b[0m \x1b[35m[ID: ${id}]\x1b[0m ${color}[BUILD ${type}]\x1b[0m ${line}`);
            }
        }
    },
